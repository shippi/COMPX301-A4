...
public class RetinalMatch {
    ...
    public static void applyMask(Mat img) {
        ...
        // creats the mask. One circle in the middle and rectangles on the top and bottom
        ...
        Mat img2 = new Mat();
        img.copyTo(img2); // copies img to img2 to store image temporarily
        img.release(); // clears img content
        img2.copyTo(img, mask); // copies the content of img2 to img within the mask
        Core.bitwise_not(img, img); // inverts the image
    }
    ...
    public static void applyCrop(Mat img) {
        ...
        for(int i = 0; i < 2; i++)  // loop that creates the translation matrix for moving the iamge
        	for(int j = 0; j < 3; j++) translation.put(i, j, translateArr[i][j]);
        	
        // applies the translation matrix and crops the image
    ...
    public static Boolean checkMatching(Mat src1, Mat src2) {
    	int numSplits = 4; // size of the square grid to split src1 into (4x4)
    	double threshhold = 0.3; // threshold for tempalte matching similarity
    	Mat template = new Mat(); // Mat to store a template for matching
    	Mat result = new Mat(); // Mat to store the result of the matching
    	
    	// loop that runs the template matching
    	for(int i = 0; i < numSplits; i++) {
    		for(int j = 0; j < numSplits; j++) {
    	    	// block of code to create the template for matching based on current iteration
    	        int[][] translateArr = {{1, 0, -(src1.cols()/numSplits) * j}, {0, 1,  -(src1.rows()/numSplits) * i}};
    	        Mat translation = new Mat(2, 3, CvType.CV_32F);
    	        for(int row = 0; row < 2; row++)
    	        	for(int column = 0; column < 3; column++) translation.put(row, column, translateArr[row][column]);
    	        ...
    	        if(mmr.maxVal > threshhold) return true;
    		}
    	}
    	return false;
    }
